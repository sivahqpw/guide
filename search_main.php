...
$seo_page = ' - '.$searchp_page_text.' ' . $_GET['p'];
} 
else { 
$page = '1';
$seo_page = '';
...
<html>
  <head>
    ...
    <meta name="description" content="<?php echo $search_page_title; ?> - <?php echo $search_seo_title;?> - <?php echo $site_title;?>"/>
	<meta property="og:title" content="<?php echo $search_page_title; ?> - <?php echo $search_seo_title;?> - <?php echo $site_title;?>" />
    <meta property="og:description" content="<?php echo $search_page_title; ?> - <?php echo $search_seo_title;?> - <?php echo $site_title;?>" />
...
<!-- Search Form -->		
  ...
  <div class="form-row align-items-center mb-3">
    <div class="col job-search">
      <input name="j" type="text" class="form-control form-control-lg" value="<?php if (!empty($_GET['j'])) { echo $search_title; }; ?>" placeholder="<?php echo $searchf_text; ?>">
    </div>
    <div class="col loc-search">
      <input name="l" type="text" class="form-control form-control-lg" value="<?php if (!empty($_GET['l'])) { echo $search_title_loc; }; ?>" placeholder="<?php echo $searchloc_text; ?>">
    </div>
...
<?php if(isset($show_sortoptions) and !empty($show_sortoptions)): ?>  
  ...
  <div style="margin-right: 10px;margin-top: -4px;"><?php echo $searchfsort_text; ?></div>
	  ...
	  <input id="relevance" name="s" value="relevance" class="custom-control-input" <?php if ($sortby == 'relevance') { echo 'checked=""';} ?> type="radio">
      <label class="custom-control-label" for="relevance"><?php echo $searchfrel_text; ?></label>
    </div>
    <div class="custom-control custom-radio mr-2">
      <input id="date" name="s" value="date" class="custom-control-input" <?php if ($sortby == 'date') { echo 'checked=""';} ?> type="radio">
      <label class="custom-control-label" for="date"><?php echo $searchfdat_text; ?></label>
    </div>
    <div class="custom-control custom-radio mr-2">
      <input id="salary" name="s" value="salary" class="custom-control-input" <?php if ($sortby == 'salary') { echo 'checked=""';} ?> type="radio">
...
<!-- /Search Container -->
  <div class="container">
	<div class="row">
		<div class="col-md-12">
<?php 
$postad_top = file_get_contents("ads/searchpage_ad_top.php",NULL);
...
<hr>
<?php
$string = file_get_contents('http://public.api.careerjet.net/search?locale_code='.$cj_local.'&keywords='.$search_term.'&location='.$search_loc.'&sort='.$sortby.'&pagesize='.$nr_search_result.'&affid='.$youtube_key.'&page='.$page.'&user_ip=66.249.65.79&user_agent=Mozilla');
...
?>
<?php 
$postad_bottom = file_get_contents("ads/searchpage_ad_bottom.php",NULL);
...
<?php if ($json->pages > 1): ?>
            <div class="pagenavbar">
                <ul class="pagination justify-content-center">
                        ...
                        elseif ($page == 2):
                            echo '<li class="page-item"><a class="page-link" rel="prev" href="' . $site_url . '/jobs?j='.$save_search.'&l='.$search_loc.'&s='.$sortby.'">&laquo;</a></li>';
                        else:
                            echo '<li class="page-item"><a class="page-link" rel="prev" href="' . $site_url . '/jobs?j='.$save_search.'&l='.$search_loc.'&s='.$sortby.'&p=' . ($page - 1) . '">&laquo;</a></li>';
                        endif;
                        ...
                        for ($i = 1; $i <= $json->pages; $i++):
                            ...
                            if ($i == 1 OR ($i > ($page - 5) and $i < ($page + 5)) OR $i == $json->pages):
                                ...
                                if ($i == 1):
								echo '<li ' . $active . '><a class="page-link" href="' . $site_url . '/jobs?j='.$save_search.'&l='.$search_loc.'&s='.$sortby.'">' . $i . '</a></li>';
								else:
                                echo '<li ' . $active . '><a class="page-link" href="' . $site_url . '/jobs?j='.$save_search.'&l='.$search_loc.'&s='.$sortby.'&p=' . $i . '">' . $i . '</a></li>';
								endif;
                                if ($i == 1 and ($page > ($i + 5))):
                                    echo '<li class="page-item disabled"><a class="page-link" href="javascript:void(0);">.....</a></li>';
                                endif;
                        ...
                        if ($page >= $json->pages):
                            ...
                            echo '<li class="page-item"><a class="page-link" rel="next" href="' . $site_url . '/jobs?j='.$save_search.'&l='.$search_loc.'&s='.$sortby.'&p=' . ($page + 1) . '">&raquo;</a></li>';
                        endif;
                        ?>
